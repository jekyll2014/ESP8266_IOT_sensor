...
#define LOG_SIZE 24 // Save sensors value each 10 minutes: ( 32hours * 60minutes/hour ) / 10minutes/pcs = 192 pcs
...
void setup()
{
	...
	if (mqttEnable)
	{
		...
		mqtt_client.connect(deviceName.c_str(), mqtt_User.c_str(), mqtt_Password.c_str());
...
void loop()
{
	...
	if (wiFiIntendedStatus != WIFI_STOP)
	{
		...
		{
			if (millis() - wifiReconnectLastTime > checkSensorPeriod)
				wifiConnect();
		}
		if (wiFiIntendedStatus == WIFI_CONNECTING && WiFi.status() == WL_CONNECTED)
		{
		...
		if (autoReport)
		{
			...
			if (WiFi.status() == WL_CONNECTED)
			{
				...
				if (mqttEnable)
				{
					...
					{
						mqtt_client.publish(mqtt_topic_out.c_str(), str.c_str());
					}
				}
#endif
...
String processCommand(String command, byte channel, bool isAdmin)
{
	...
	if (mqttEnable)
	{
		mqtt_client.setServer(mqtt_server.c_str(), mqtt_port);
		mqtt_client.setCallback(mqtt_callback);
		mqtt_client.connect(mqtt_device_name.c_str(), mqtt_User.c_str(), mqtt_Password.c_str());
		mqtt_client.subscribe(mqtt_topic_in.c_str());
	}
		*/
		else if (tmp.startsWith(F("mqtt_server=")) && command.length() > 12)
		{
			mqtt_server = command.substring(command.indexOf('=') + 1);
			str = F("New MQTT server address = \"");
			str += mqtt_server;
			str += quote;
			str += eol;
